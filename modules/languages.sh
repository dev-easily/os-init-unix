...
install_rust() {
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY-RUN] would: setup mirrors, install rustup, download toolchain, configure cargo, install cargo tools"
        log_success "[DRY-RUN] Rust installation completed (simulated)"
        return 0
    fi
...
install_nodejs() {
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY-RUN] would: install nvm via brew, install LTS node, configure npm mirrors, install pnpm/yarn"
        log_success "[DRY-RUN] Node.js installation completed (simulated)"
        return 0
    fi
...
install_python() {
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY-RUN] would: install pyenv via brew, install Python 3.11/3.12, configure pip mirror, install poetry/pipenv"
        log_success "[DRY-RUN] Python installation completed (simulated)"
        return 0
    fi
...
install_golang() {
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY-RUN] would: install go via brew, create GOPATH directories, configure GOPROXY, install dev tools"
        log_success "[DRY-RUN] Go installation completed (simulated)"
        return 0
    fi
...
install_java() {
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY-RUN] would: install openjdk via brew, install maven/gradle, configure maven mirror"
        log_success "[DRY-RUN] Java installation completed (simulated)"
        return 0
    fi
...
install_php() {
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY-RUN] would: install php composer via brew, configure composer mirror"
        log_success "[DRY-RUN] PHP installation completed (simulated)"
        return 0
    fi
...
install_ruby() {
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY-RUN] would: install rbenv via brew, install latest ruby, configure gem mirror, install bundler/rails"
        log_success "[DRY-RUN] Ruby installation completed (simulated)"
        return 0
    fi
...
install_flutter() {
    ...
    if [ "$DRY_RUN" = "true" ]; then
        log_info "[DRY-RUN] would: download flutter SDK, configure mirrors, install cocoapods (macOS), run flutter doctor"
        log_success "[DRY-RUN] Flutter installation completed (simulated)"
        return 0
    fi
    ...
    local flutter_url
    if is_macos; then
        flutter_url="https://storage.flutter-io.cn/flutter_infra_release/releases/stable/macos/flutter_macos${package_arch}_${flutter_version}-stable.zip"
    elif is_ubuntu; then
    ...
    if safe_download "$flutter_url" "$flutter_zip"; then
        ...
        # 安装CocoaPods (iOS开发需要 - macOS only)
        if is_macos && ! command -v pod >/dev/null 2>&1; then
